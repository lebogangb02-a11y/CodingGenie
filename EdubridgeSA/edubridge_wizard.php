    ...
    try {
        ...
        if ($appId > 0) {
            ...
            try {
                ...
                $sA->execute([$appId]);
                foreach ($sA->fetchAll(PDO::FETCH_COLUMN) as $t) {
                    $uploaded[] = strtolower(trim((string)$t));
                }
            ...
            try {
                ...
                $sB->execute([$appId]);
                foreach ($sB->fetchAll(PDO::FETCH_COLUMN) as $t) {
                    $uploaded[] = strtolower(trim((string)$t));
                }
            } catch (Throwable $e) { /* ignore */
            }
...
function handle_upload($field, $subdir)
{
    ...
    $res = store_uploaded_file($file, $subdir, ['application/pdf', 'image/jpeg', 'image/png'], MAX_FILE_SIZE);
    if (!$res['success']) throw new Exception('Failed to store file for ' . $field . ': ' . ($res['error'] ?? 'unknown'));
    return $res['path'];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($step === 1) {
            ...
            $combos = [];
            $norm = function ($u, $c) {
                return strtolower(trim($u)) . '|' . strtolower(trim($c));
            };
            ...
            if ($institution_choice_2 && $program_choice_2) {
                ...
                foreach ($combos as $cx) {
                    if ($cx === $c2) throw new Exception('Your second choice duplicates another selection. Please choose a different university/course combination.');
                }
                $combos[] = $c2;
            }
            if ($institution_choice_3 && $program_choice_3) {
                ...
                foreach ($combos as $cx) {
                    if ($cx === $c3) throw new Exception('Your third choice duplicates another selection. Please choose a different university/course combination.');
                }
                $combos[] = $c3;
            }
...
<body class="bg-light">
    <div class="container py-4">
        ...
        <?php if ($errors): ?><div class="alert alert-danger"><?php foreach ($errors as $err) {
                        ...
                        <div class="row g-3">
                            <div class="col-md-3"><label class="form-label">Title</label>
                                <select name="title" class="form-select">
                                    <?php $titles = ['', 'Mr', 'Ms', 'Mrs', 'Dr'];
                                    $cur = $app['title'] ?? '';
                                    foreach ($titles as $t) {
                                        $sel = $cur === $t ? 'selected' : '';
                                        echo "<option value=\"$t\" $sel>" . ($t ?: 'Select Title') . "</option>";
                                    } ?>
                                </select>
                            </div>
                            ...
                            <div class="col-md-6"><label class="form-label">Gender</label><select name="gender" class="form-select">
                            ...
                            <div class="col-md-6"><label class="form-label required">Province</label><select name="province" class="form-select" required>
                                    <?php $provList = ['Eastern Cape', 'Free State', 'Gauteng', 'KwaZulu-Natal', 'Limpopo', 'Mpumalanga', 'Northern Cape', 'North West', 'Western Cape'];
                                    $cur = $app['province'] ?? '';
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="matric_year" id="matric_year" class="form-select" required>
                                    ...
                                    for ($y = $currentYear; $y >= 1980; $y--) {
                                        $sel = ((int)($app['matric_year'] ?? 0) === $y) ? 'selected' : '';
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <select id="course_category" class="form-select">
                                    ...
                                    <?php $cats = ['Education', 'Business', 'IT & Computing', 'Health & Sciences', 'Engineering', 'Humanities', 'Art & Design'];
                                    foreach ($cats as $c) {
                                ...
                                <div class="table-responsive">
                                    <table class="table table-sm">
                                        <tbody>
                                            <tr>
                                                <th>Full Name</th>
                                                <td><?= htmlspecialchars(($app['title'] ?? '') ? ($app['title'] . ' ') : '') ?><?= htmlspecialchars($app['full_name'] ?? '') ?> <?= htmlspecialchars($app['surname'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>Email</th>
                                            ...
                                            <tr>
                                                <th>Cellphone</th>
                                                <td><?= htmlspecialchars($app['cellphone_number'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>Home Language</th>
                                                <td><?= htmlspecialchars($app['home_language'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>Nationality</th>
                                                <td><?= htmlspecialchars($app['nationality'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>Province</th>
                                                <td><?= htmlspecialchars($app['province'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>School Province</th>
                                                <td><?= htmlspecialchars($app['school_province'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>City</th>
                                                <td><?= htmlspecialchars($app['city'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>Country of Residence</th>
                                                <td><?= htmlspecialchars($app['country_of_residence'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>Address</th>
                                                <td><?= htmlspecialchars($app['physical_address'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>Postal Code</th>
                                                <td><?= htmlspecialchars($app['postal_code'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>High School</th>
                                                <td><?= htmlspecialchars($app['high_school_name'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>Matric Year</th>
                                                <td><?= htmlspecialchars($app['matric_year'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>APS</th>
                                                <td><?= htmlspecialchars($app['aps'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>Maths Level</th>
                                                <td><?= htmlspecialchars($app['maths_level'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>English Level</th>
                                                <td><?= htmlspecialchars($app['english_level'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>Exam Number</th>
                                                <td><?= htmlspecialchars($app['exam_number'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>University</th>
                                                <td><?= htmlspecialchars($app['institution_choice_1'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>Course</th>
                                                <td><?= htmlspecialchars($app['program_choice_1'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>First Choice Specialization</th>
                                                <td><?= htmlspecialchars($app['program_specialization_1'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>Second Choice University</th>
                                                <td><?= htmlspecialchars($app['institution_choice_2'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>Second Choice Course</th>
                                                <td><?= htmlspecialchars($app['program_choice_2'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>Second Choice Specialization</th>
                                                <td><?= htmlspecialchars($app['program_specialization_2'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>Third Choice University</th>
                                                <td><?= htmlspecialchars($app['institution_choice_3'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>Third Choice Course</th>
                                                <td><?= htmlspecialchars($app['program_choice_3'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>Third Choice Specialization</th>
                                                <td><?= htmlspecialchars($app['program_specialization_3'] ?? '') ?></td>
                                            </tr>
                                            <tr>
                                                <th>Reference</th>
                                                <td><?= htmlspecialchars($_SESSION['reference_number'] ?? '') ?></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function handleIdChange(val) {
                ...
                if (parentSection && age !== null) {
                    parentSection.style.display = (age < 18 ? 'block' : 'none');
                }
            }

            idInput.addEventListener('input', function(e) {
                handleIdChange(e.target.value.trim());
            });
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var eduEl = document.querySelector('[name="education_status"]');
            var schoolEl = document.querySelector('[name="school_type"]');
            [incomeEl, perfEl].forEach(function(el) {
                if (el) {
                    el.addEventListener('change', checkBursaryEligibility);
                }
            });
            [eduEl, schoolEl].forEach(function(el) {
                if (el) {
                    el.addEventListener('change', calculateApplicationFee);
                }
            });
            // Initial evaluations
