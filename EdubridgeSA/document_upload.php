...
function getUploadDirFor($field) {
    switch ($field) {
        case 'certified_id':
            return defined('UPLOAD_ID_DIR') ? UPLOAD_ID_DIR : (defined('UPLOAD_DIR') ? UPLOAD_DIR . 'id_documents/' : 'uploads/applications/id_documents/');
        case 'proof_of_residence':
            return defined('UPLOAD_RESIDENCE_DIR') ? UPLOAD_RESIDENCE_DIR : (defined('UPLOAD_DIR') ? UPLOAD_DIR . 'proof_of_residence/' : 'uploads/applications/proof_of_residence/');
        case 'parent_guardian_id':
            return defined('UPLOAD_PARENT_ID_DIR') ? UPLOAD_PARENT_ID_DIR : (defined('UPLOAD_DIR') ? UPLOAD_DIR . 'parent_guardian_id/' : 'uploads/applications/parent_guardian_id/');
        case 'academic_results':
            return defined('UPLOAD_ACADEMIC_DIR') ? UPLOAD_ACADEMIC_DIR : (defined('UPLOAD_DIR') ? UPLOAD_DIR . 'academic_results/' : 'uploads/applications/academic_results/');
        default:
            return defined('UPLOAD_BASE_DIR') ? UPLOAD_BASE_DIR : (defined('UPLOAD_DIR') ? UPLOAD_DIR : 'uploads/applications/');
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload_documents'])) {
    ...
    try {
        $pdo = getDBConnection();
        // Fetch full application details to avoid undefined keys in later rendering
        $stmt = $pdo->prepare("SELECT * FROM applications WHERE reference_number = ?");
        $stmt->execute([$reference_number]);
        ...
        if (!$applicationRow) {
            ...
            // Preserve full application details for UI rendering below
            ...
            if (!empty($uploaded_files)) {
                // The applications table does not have a 'documents_uploaded' column.
                // Use the existing 'status' column to reflect the upload event.
                $stmt = $pdo->prepare("UPDATE applications SET status = 'Submitted (with docs)', updated_at = NOW() WHERE id = ?");
                $stmt->execute([$application_id]);

                // Log status change (optional): only if table exists; do not block uploads
                try {
                    ...
                    // Swallow logging errors to avoid user-facing banner; keep for server logs
