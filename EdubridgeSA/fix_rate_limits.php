...
try {
    ...
    if (!$table_exists) {
        ...
        if ($ip_column) {
            ...
            $cols = "id, $ip_column AS ip_address, action_type, attempt_count, window_start, created_at";
            $stmt = $pdo->prepare("SELECT $cols FROM rate_limits WHERE $ip_column = ? ORDER BY created_at DESC LIMIT 100");
