...
try {
    ...
    if (isset($resolvedId) && $resolvedId) {
        ...
        if ($contactEmail) {
            ...
            $stmt->execute([$contactEmail]);
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($row && !empty($row['id'])) { $application_id = (int)$row['id']; }
        }
        if (!$application_id) {
            $sessionRef = $_SESSION['reference_number'] ?? null;
            if ($sessionRef) {
                ...
                if ($row2 && !empty($row2['id'])) { $application_id = (int)$row2['id']; }
            }
        }
    }
    ...
    if ($application_id) {
        ...
        if ($application) { $completed_steps++; }
        ...
        if ($pg && !empty($pg['full_name'])) { $completed_steps++; }
        ...
        if ($uc && !empty($uc['university_1'])) { $completed_steps++; }
        ...
        $uploaded_types = [];
        foreach ($docs as $doc) { if (!empty($doc['document_type'])) { $uploaded_types[] = $doc['document_type']; } }
        ...
        if ($has_all_required_docs) { $completed_steps++; }
        ...
        if ($has_all_required_docs) { $completion_percentage = 100; }
    } else {
    ...
    if ($completion_percentage <= 0) { $completion_percentage = 20; }
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        :root {
            /* EduBridge SA theme */
            --primary: #004AAD; /* EduBridge Blue */
            --secondary: #4CAF50; /* EduBridge Green */
            --light: #f5f7fa;
            --dark: #1a237e;
            --accent: #F9FAFB; /* Soft white surface */
            --royal-blue: #1e3a8a;
        ...
        * {
        ...
        .progress-visual { justify-content: center; }
        .progress-ring { width: 160px; height: 160px; position: relative; margin: 10px auto; }
        .progress-ring svg { width: 160px; height: 160px; }
        .progress-ring circle.bg { stroke: var(--gray-200); stroke-width: 12; fill: none; }
        .progress-ring circle.fg { stroke: url(#ringGradient); stroke-width: 12; fill: none; stroke-linecap: round; transform: rotate(-90deg); transform-origin: 50% 50%; transition: stroke-dashoffset .8s ease; }
        .progress-ring__text { position: absolute; inset: 0; display:flex; align-items:center; justify-content:center; font-weight:800; font-size:1.8rem; color: var(--gray-800); }
        .progress-ring.glow { filter: drop-shadow(0 4px 12px rgba(76,175,80,.2)); }
        ...
        .hero-title { font-size: 1.6rem; font-weight: 700; margin: 0 0 8px; }
        .hero-sub { font-size: 1rem; opacity: .95; }
        ...
        .journey-item:hover { transform: translateY(-4px); box-shadow: 0 12px 28px rgba(0,0,0,.18); }
        .journey-item img { width: 100%; height: 140px; object-fit: cover; display: block; }
        .journey-caption { position: absolute; left: 0; right: 0; bottom: 0; padding: 10px 12px; color: #fff; font-weight: 600; font-size: .9rem; background: linear-gradient(180deg, rgba(0,0,0,0) 0%, rgba(0,0,0,.55) 100%); }

        /* Smart Progress Widget */
        .smart-progress-card .smart-header {
            display:flex; align-items:center; justify-content:space-between; gap:10px; margin-bottom:8px;
        }
        .smart-progress-card .smart-title { margin:0; font-size:1.25rem; color: var(--gray-800); }
        .smart-badge { padding:6px 12px; border-radius:999px; color:#fff; font-size:0.8rem; font-weight:600; }
        .smart-badge.track { background: var(--primary); }
        .smart-badge.ahead { background: var(--secondary); }
        .smart-badge.slight { background: var(--accent); }
        .smart-badge.delay { background: #c62828; }
        .smart-bar { height: 10px; background: var(--gray-200); border-radius: 6px; overflow: hidden; margin: 10px 0 12px; }
        .smart-fill { height: 100%; background: linear-gradient(90deg, var(--primary), var(--secondary)); width: 0; transition: width .4s ease; }
        .smart-meta { color: var(--gray-600); font-size: 0.9rem; }
        .smart-grid { display:grid; grid-template-columns: repeat(4, 1fr); gap:12px; margin-top: 12px; }
        .smart-stage { border:1px solid var(--gray-200); border-radius:10px; padding:12px; }
        .smart-stage .label { font-weight:600; color: var(--gray-800); margin-bottom:4px; }
        .smart-stage .muted { color: var(--gray-600); font-size:0.85rem; }
        ...
        body.dark .card { background: #0f1f3b; border-color: #1f3a66; box-shadow: 0 8px 25px rgba(0,0,0,.35); }
        body.dark .user-info { background: linear-gradient(135deg, #0c1a36 0%, #12305f 100%); }
...
<body>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function applyMetrics(data) {
            ...
            } catch (e) { console.error('Apply metrics error', e); }
        }

        function fetchMetrics() {
            fetch('get-dashboard-metrics.php', { credentials: 'same-origin' })
                .then(function(r){ return r.json(); })
                .then(function(data){ if (data && data.success) { applyMetrics(data); } })
                .catch(function(err){ console.error('Metrics fetch failed', err); });
        }
    ...
    document.addEventListener('DOMContentLoaded', function(){
        ...
        if (window.Typed) {
            if (motivatorEl) {
                new Typed('#motivatorText', { strings: msgs, typeSpeed: 40, backSpeed: 25, backDelay: 1800, loop: true });
            }
            ...
            if (document.getElementById('quoteTyped')) {
                new Typed('#quoteTyped', { strings: quoteMsgs, typeSpeed: 35, backSpeed: 20, backDelay: 2200, loop: true });
            }
        } else {
        ...
        if (window.lottie && document.getElementById('lottieProgress')) {
            try {
                lottie.loadAnimation({ container: document.getElementById('lottieProgress'), renderer: 'svg', loop: true, autoplay: true, path: lottieUrl });
            } catch(e) {}
        }
